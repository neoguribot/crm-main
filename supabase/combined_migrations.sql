-- 제일금거래소 CRM — 전체 마이그레이션 통합본 (0001~0030)
-- Supabase 대시보드 > SQL Editor 에 그대로 붙여넣고 실행
-- 각 마이그레이션은 멱등이라 재실행 안전
--
-- 마케팅 자동화(0027~0030)만 적용하려면 supabase/marketing_0027-0030.sql 을
-- 쓰거나, 이 파일 맨 아래 "0027_message_templates.sql" 섹션부터 끝까지 복사해 실행.
...
-- ============================================================
-- 0026_customer_analytics_referrers.sql
-- ============================================================

-- =====================================================================
-- 0026_customer_analytics_referrers.sql
-- 종합 분석 "상위 고객"에 최다 추천 고객 추가:
-- - top_referrers: 자기를 추천인(referred_by_customer_id)으로 지정한 다른
--   고객 수가 많은 순 상위 10명. { id, name, referral_count }
--
-- customer_analytics() 는 security invoker 라 호출자 RLS 로 자기 고객만 집계한다.
-- 여러 번 실행해도 안전. 되돌리기는 supabase/README.md 참고.
-- =====================================================================
...
as $$
  ...
  top_referrers as (
    select r.id, r.name, count(c.id) as referral_count
    from customers r
    join customers c on c.referred_by_customer_id = r.id
    group by r.id, r.name
    order by count(c.id) desc, r.name
    limit 10
  ),
  customer_trade_counts as (
    select
